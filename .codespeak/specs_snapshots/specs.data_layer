...
PostgreSQL database accessed via SQLAlchemy (async) with Alembic for migrations. All data is scoped per user via a `user_id` string on every top-level entity. The ORM uses a shared `Base` class; an async session factory is provided as a FastAPI dependency (`get_db`). Migrations support both offline and online (async) modes; the `DATABASE_URL` environment variable overrides the URL in `alembic.ini`, enabling Docker/CI usage without file edits.
...
### Cardio Segment
Represents an interval within a cardio session (e.g. a lap or a pace zone). Fields: `order`, `duration_seconds` (required), optional `distance_meters`, `pace_seconds_per_km`, `heart_rate_avg`, `title`.
...
### StrengthTemplate
Per-user, has `name`, optional `notes`, `created_at`, `updated_at`. Contains an ordered list of **StrengthTemplateExercises**.

### StrengthTemplateExercise
References an `Exercise` (delete restricted). Contains an ordered list of **StrengthTemplateSets**.

### StrengthTemplateSet
`set_number`, optional `reps`, `weight_kg` (float), optional `notes`. Template sets store weight as `weight_kg`; session sets store it as `weight` (unit-agnostic).
